...
def fe(s, start):
    ...
    while i < len(s) and d > 0:
        ...
        if s[i:i+2] == '--':
            ...
            i = nl + 1
            continue
        if s[i:i+3] == 'end' and (i == 0 or not (s[i-1].isalnum() or s[i-1] == '_')) \
           and (i+3 >= len(s) or not (s[i+3].isalnum() or s[i+3] == '_')):
            d -= 1
            i += 3
            if d == 0: return i
            continue
        if s[i:i+8] == 'function' and (i == 0 or not (s[i-1].isalnum() or s[i-1] == '_')):
            d += 1; i += 8; continue
        if s[i:i+2] == 'if' and (i == 0 or not (s[i-1].isalnum() or s[i-1] == '_')) \
           and (i+2 >= len(s) or not (s[i+2].isalnum() or s[i+2] == '_')):
            d += 1; i += 2; continue
        if s[i:i+3] == 'for' and (i == 0 or not (s[i-1].isalnum() or s[i-1] == '_')) \
           and (i+3 >= len(s) or not (s[i+3].isalnum() or s[i+3] == '_')):
            d += 1; i += 3; continue
        if s[i:i+5] == 'while' and (i == 0 or not (s[i-1].isalnum() or s[i-1] == '_')) \
...
def ae(s):
    sc = {}
    for m in re.finditer(r'local\s+(\w+)\s*=\s*(\d+)\b', s):
        sc[m.group(1)] = int(m.group(2)) & 255
    for m in re.finditer(r'local\s+(\w+)\s*=\s*\(\s*([\d+\-*%()\s]+?)\s*\)', s):
        n = m.group(1)
        if n not in sc:
            try: sc[n] = int(eval(m.group(2))) & 255
            except: pass

    tb = {}
    for m in re.finditer(r'local\s+(\w+)\s*=\s*\{', s):
        n = m.group(1)
        if n in tb: continue
        r = br(s, n)
        if not r: continue
        try:
            a = ar(r)
            if isinstance(a, list) and a: tb[n] = a
        except: pass
    ...
    m = re.search(
        ...
        r'for\s+\w+\s*=\s*1\s*,\s*#(\w+)\s+do', s)
    if not m: return None
    ua, ub, st = m.group(1), m.group(2), m.group(3)
    ...
    m = re.search(
        ...
        r'while\s+\w+\s*<=\s*#(\w+)\s+do', s)
    if not m: return None
    ba, bb, bt = m.group(1), m.group(2), m.group(3)

    m = re.search(
        r'OG21\s*\(\s*(\w+)\s*\[\s*\w+\s*\]\s*or\s*0\s*,\s*(\w+)\s*\)', s)
    if not m: return None
    ot, ok = m.group(1), m.group(2)

    m = re.search(
        r'OG21\s*\(\s*(\w+)\s*\[\s*\w+\s*\]\s*or\s*0\s*,\s*'
        r'OG21\s*\(\s*(\w+)\s*,', s)
    if not m: return None
    lt, lk = m.group(1), m.group(2)

    m = re.search(
        r'local\s+\w+\s*=\s*\(\s*(\w+)\s*\+\s*\(\s*\w+\s*-\s*1\s*\)\s*'
        r'\*\s*(\w+)\s*\)\s*%\s*256', s)
    if not m: return None
    s1, s2 = m.group(1), m.group(2)

    zm = {}
    for m in re.finditer(r'(\w+)\s*\[\s*(\d+)\s*\]\s*=\s*function\s*\([^)]*\)', s):
        num = int(m.group(2))
        ep = fe(s, m.end())
        if ep == -1:
            nl = s.find('\n', m.end())
            if nl == -1: nl = len(s)
            b = s[m.end():nl].strip()
            if b.endswith('end'): b = b[:-3].strip()
        else:
            b = s[m.end():ep].strip()
            if b.endswith('end'): b = b[:-3].strip()
        c = None
        if re.search(r'=\s*function\s*\(\s*\.\.\.', b):                    c = 29
        elif '_G' in b or 'getfenv' in b:                                  c = 27
        elif re.search(r'\breturn\b', b):                                  c = 23
        elif re.search(r'\bfor\b', b):                                     c = 22
        elif re.match(r'^\w+\s*\[\s*\w+\s*\]\s*=\s*\w+\s*\[\s*\w+\s*\]$', b): c = 1
        elif re.match(r'^\w+\s*\[\s*\w+\s*\]\s*=\s*\w+\s*\(', b):          c = 2
        if c is not None: zm[num] = c

    for n in (ua, ub, ba, bb, ok, lk, s1, s2):
        if n not in sc: return None
    for n in (st, bt):
        if n not in tb: return None

    return {
        'A':  tb[st], 'B': tb[bt],
        'Pi': tb.get(ot, []), 'HY': tb.get(lt, []),
        'Up': sc[ua], 'CO': sc[ub],
        'p3': sc[ba], 'o2': sc[bb],
        's5': sc[ok], 'hm': sc[lk],
        'a4': sc[s1], 'x8': sc[s2],
            ...
            while i < j0:
                ...
                if R == 20 or R == 21:
                    ...
                    if el is not None:
                        ...
                        if R == 21: o.append(pr + "if " + cd + " then")
                        else:       o.append(pr + "if not " + cd + " then")
                        em(i + 1, tg, dd + 1, dict(t))
                        o.append(pr + "end")
                        i = tg
                    t.clear(); continue
                ...
                if R == 1:
                    ...
                    o.append(pr + "%s[%s] = %s" % (sy(t, a), sy(t, b), sy(t, c)))
                elif R == 27:
                    g = lk(p, b)
                    if isinstance(g, str) and ir.match(g):
                        sr(t, a, g)
                    ...
                    if va > 0: prs.append("...")
                    if not prs: prs.append("...")
                    o.append(pr + "local function %s(%s)" % (fn, ", ".join(prs)))
